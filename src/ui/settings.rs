...
pub struct SettingsWindow {
    eng_input: Option<Entity<TextInput>>,
...
impl SettingsWindow {
    ...
    pub fn open(cx: &mut App) -> gpui::Result<()> {
        let bounds = Bounds::centered(None, size(px(820.0), px(520.0)), cx);
        cx.open_window(
            WindowOptions {
                window_bounds: Some(WindowBounds::Windowed(bounds)),
                ..Default::default()
            },
            |_, cx| cx.new(|_| SettingsWindow::new()),
        )?;
        Ok(())
    }

    fn ensure_inputs(&mut self, cx: &mut Context<Self>) {
        if self.eng_input.is_none() {
            let eng = cx.new(|cx| TextInput::new(cx, "notepad.exe, code.exe"));
            let chn = cx.new(|cx| TextInput::new(cx, "wechat.exe, chrome.exe"));
            self.eng_input = Some(eng);
            self.chn_input = Some(chn);
            self.load_existing_config(cx);
        }
    }
    ...
    fn on_save_click(&mut self, _: &MouseUpEvent, _window: &mut Window, cx: &mut Context<Self>) {
        let eng_text = self
            .eng_input
            .as_ref()
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .gap_3()
                    .flex_1()
                    .child(div().text_sm().child("英文应用（逗号分隔）"))
                    ...
                    .child(
                        div()
                            ...
                            .w(px(120.0))
                            .child("保存设置")
                            .hover(|style| style.bg(rgb(0x1f56b8)))
                            .on_mouse_up(MouseButton::Left, cx.listener(Self::on_save_click)),
                    ),
            )
    }
}
...
fn text_input_view(input: Entity<TextInput>) -> impl gpui::IntoElement {
    div().child(TextElement { input })
}
...
impl Element for TextElement {
    ...
    fn request_layout(
        ...
        style.size.width = px(500.0).into();
